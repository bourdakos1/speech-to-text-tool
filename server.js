...
app.post('/api/delete_corpus', function(req, res) {
    var username = req.query.username;
    var password = req.query.password;

    request.del('https://stream.watsonplatform.net/speech-to-text/api/v1/customizations/' + encodeURIComponent(req.query.customization_id) + '/corpora/' + encodeURIComponent(req.query.corpus))
    .auth(username, password)
    .end(function(err, response) {
        if (err) {
            res.send(err);
